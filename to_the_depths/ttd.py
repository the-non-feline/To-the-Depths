...
class TTD_Bot(discord.Client, storage.Deconstructable): 
    ...
    async def save(self, safely_shutdown=False): 
        with self.tuning_out(): 
            async with self.pulse_presence(status=discord.Status.do_not_disturb, activity=discord.Game('saving')): 
                ...
                pickle_dump(self.safely_shutdown_file, safely_shutdown) 
    ...
    async def do_on_turn_on(self): 
        should_continue = pickle_load(self.safely_shutdown_file, False) 
