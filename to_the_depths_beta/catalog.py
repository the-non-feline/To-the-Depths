...
class Cryomancer(Player): 
    ...
    specials = (f'Frostbite - when {name} is below {fb_threshold * 100:.1}% HP, damage taken from enemy attacks is reduced by \
{(1 - fb_eam) * 100:.1}%, and its own damage is increased by {(fb_attack_multiplier - 1) * 100:.1}%. This effect disappears when {name} \
goes above {fb_threshold * 100:.1}% HP. ',) 
    starting_hp = 110
    starting_attack = 40
    ...
    async def hp_changed(self, report): 
        ...
        if self.current_hp / self.max_hp <= self.fb_threshold: 
            if not self.fb_activated: 
                ...
                report.add(f'{self.name} now takes {(self.fb_eam - 1) * 100:+.1}% damage from enemy attacks! ') 
            ...
            report.add(f'{self.name} no longer takes {(self.fb_eam - 1) * 100:+.1}% damage from enemy attacks. ') 
